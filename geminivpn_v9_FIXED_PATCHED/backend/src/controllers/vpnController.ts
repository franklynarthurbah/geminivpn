...
// vpnEngine imported via singleton (avoids circular)
...
export const createClient = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const { clientName = req.body.name, serverId } = req.body;  // accept both 'clientName' (mobile apps) and 'name' (direct API)
