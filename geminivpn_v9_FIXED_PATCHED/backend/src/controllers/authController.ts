 ...
 */
const generateAccessToken = (userId: string, email: string, subscriptionStatus: string): string => {
  return jwt.sign(
 ...
 */
const generateTokenPair = (userId: string, email: string, subscriptionStatus: string): TokenPair => {
...
export const register = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // Always store email in lowercase so login lookups are consistent
    const normalizedEmail = email.trim().toLowerCase();

    // Check if user already exists
    // NOTE: email is always stored/compared in lowercase so no mode:'insensitive'
    // needed — SQLite does not support that Prisma feature anyway.
...
export const login = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    // Find user — normalise to lowercase so login works regardless of
    // how the user capitalises their email (e.g. Test@Gmail.com → test@gmail.com)
    const normalizedEmail = email.trim().toLowerCase();
