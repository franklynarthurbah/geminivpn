...
export class VPNEngine {
  ...
  private async generateKeyPair(): Promise<{ privateKey: string; publicKey: string }> {
    ...
    // Mock keys for dev/test — use Node.js crypto.randomBytes (not Web Crypto API)
    const mockPriv = crypto.randomBytes(32).toString('base64');
    const mockPub  = crypto.randomBytes(32).toString('base64');
