...
const RENDERER_URL = IS_DEV ? 'http://localhost:3001' : `file://${path.join(__dirname, '../../dist/renderer/index.html')}`;
...
function registerIpcHandlers() {
    ...
    ipcMain.handle('auth:saveTokens', async (_, { accessToken, refreshToken }) => {
        store.set('credential_' +  'access_token',  accessToken);
        store.set('credential_' +  'refresh_token', refreshToken);
    });

    ipcMain.handle('auth:getTokens', async () => {
        const accessToken  = store.get('credential_' +  'access_token');
        const refreshToken = store.get('credential_' +  'refresh_token');
        return { accessToken, refreshToken };
    });

    ipcMain.handle('auth:clearTokens', async () => {
        store.delete('credential_' +  'access_token');
        store.delete('credential_' +  'refresh_token');
    });
