/**
 * Connection Monitor — circular dependency with server.ts broken via lazy getter
 */
...
// Lazy getter breaks the circular dependency:
// server.ts → connectionMonitor.ts → server.ts (was circular, crashed module load)
// Now: server.ts → connectionMonitor.ts (uses getter at CALL TIME, not load time)
const getVpnEngine = () => {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const { vpnEngine } = require('../server');
  return vpnEngine;
};
