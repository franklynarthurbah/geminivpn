 ...
 *
 * Handles 60-minute auto-expiring demo account provisioning,
 * abuse detection, cleanup, and conversion tracking.
 *
 * ADD TO backend/src/controllers/demoController.ts
 * ADD ROUTE in backend/src/routes/demo.ts
 * ADD TO server.ts: import demoRoutes and app.use('/api/v1/demo', demoRoutes)
 */
...
export async function runDemoCleanup(): Promise<number> {
  ...
  for (const demo of expiredDemos) {
    try {
      ...
      // 2. Mark demo as deleted (soft delete for analytics retention)
